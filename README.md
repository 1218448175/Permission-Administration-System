...
本项目是一套基于 **Django 5** 和 **Vue 3** 开发的前后端分离权限管理系统。项目定位为高级 Python 就业实战课程，旨在提高开发者在前后端分离架构下的实战水平 。系统通过 **JWT (JSON Web Token)** 技术实现状态无关的身份验证，并提供了完整的用户、角色、权限管理功能 。已部署：[RBAC用户权限系统](http://123.56.116.79/#/login)
